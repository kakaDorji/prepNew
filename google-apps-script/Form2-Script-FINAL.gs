// ==========================
// GOOGLE APPS SCRIPT FOR FORM 2 - FINAL VERSION
// With header mapping to fix prefill issues
// ==========================
...
function getPrefillData(uid) {
  try {
    ...
    console.log('Mapped prefill data for UID:', uid);
    console.log('Found fields:', Object.keys(prefill));
    return prefill;
  } catch (error) {
...
function saveOrUpdateData(data) {
  try {
    ...
    const rowData = headers.map(header => {
      // Try to find the value using the mapping
      let value = "";
    ...
    console.log('Saving data with header mapping for UID:', uid);
...
function handleRequest(e) {
  ...
  try {
    console.log("Request received:", { action: data.action, uid: data.participant_uid });
...
function testPrefillTHI0009() {
  const result = getPrefillData("THI0009");
  console.log("Test prefill result for THI0009:");
  console.log("Found data:", result ? "YES" : "NO");
  if (result) {
    console.log("Fields found:", Object.keys(result).slice(0, 10)); // Show first 10 fields
  }
  return result;
}

function testRequestTHI0009() {
  const mockEvent = {
    parameter: {
