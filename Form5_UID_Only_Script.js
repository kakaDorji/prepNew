...
function getColumnMapping() {
  return {
    ...
    36: 'Designation',
    37: 'BMHC Number:' // Updated to include colon as per your headers
  };
}
...
function getPrefillData(uid) {
  try {
    ...
    const data = fetchByUid(uid);
    if (!data) return { status: 'error', message: `No unscheduled visit data found for ${uid}` };
    ...
    const fieldMapping = {
      ...
      'stopping_indicatte_reasons_ preferred_method': 'stopping_Preventionmethod', // Updated with space
      'Other_notes': 'other_notes',
      'Date': 'interviewer_date',
      'Name': 'interviewer_name',
      'Designation': 'designation',
      'BMHC Number:': 'bmhc_number', // Updated with colon as per headers
      'Nausea': 'nausea',
      ...
      'Abdominalcramps_ Resolved': 'abdominal_cramps_resolved', // Updated with space
      'Fatigue': 'fatigue',
...
// Create the row data array for unscheduled visit (matching spreadsheet column order)
function createRowData(e) {
  return [
    ...
    safeGetParameter(e, 'stopping_indicate_reasons_ preferred_method', ''),
...
function addNewRecord(e) {
  try {
    ...
    if (existingData) {
      return { status: 'error', message: `Unscheduled visit record already exists for UID: ${uid}. Use update instead.` };
    }
...
function updateRecord(e) {
  try {
    ...
    const existingData = fetchByUid(uid);
    if (!existingData) {
      return { status: 'error', message: `Unscheduled visit record for UID ${uid} not found` };
    }
