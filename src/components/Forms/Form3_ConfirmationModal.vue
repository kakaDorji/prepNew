    ...
    <div class="modal-overlay" @click.self="emitCancel">
      <div class="modal-content enhanced-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title-form3">
        ...
        <div class="modal-header">
          <h2 id="modal-title-form3">
            <i class="fas fa-notes-medical"></i> Review Form 3 Details
          </h2>
        ...
        <div class="modal-body">
          <p class="modal-instruction">
            Please confirm the following information before submitting Form 3:
          </p>
          ...
          <div v-if="formData" class="modal-details-grid">
            ...
            <div class="detail-item">
              ...
              <span class="detail-value">{{
                formData.visit_type || "Not Selected"
                }}</span>
            </div>

            <!-- Section 1: Acute HIV Infection -->
            <h4 class="details-section-title">
              1. Acute HIV Infection Symptoms (Past 15 Days)
            </h4>
            <template v-for="symptom in acuteSymptoms" :key="symptom.key">
              <div v-if="formData[symptom.key] === 'Yes'" class="symptom-group">
                ...
                <div class="detail-item">
                  <span class="detail-label">Specify:</span><span class="detail-value">{{
                    formData[symptom.key + "_specify"] || "N/A"
                    }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Start Date:</span><span class="detail-value">{{
                    formData[symptom.key + "_start_date"] || "N/A"
                    }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Ongoing:</span><span class="detail-value">{{
                    formData[symptom.key + "_ongoing"] ? "Yes" : "No"
                    }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Resolved:</span><span class="detail-value">{{
                    formData[symptom.key + "_resolved"] ? "Yes" : "No"
                    }}</span>
                </div>
                <div class="detail-item">
                  <span class="detail-label">Present on Exam:</span><span class="detail-value">{{
                    formData[symptom.key + "_present"] ? "Yes" : "No"
                    }}</span>
                </div>
              </div>
            </template>
            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-exclamation-circle"></i> Recent HIV
                Exposure:</span>
              <span class="detail-value">{{
                formData.recent_hiv_exposure || "N/A"
                }}</span>
            </div>
            <div v-if="formData.recent_hiv_exposure === 'Yes'" class="detail-item full-width">
              ...
              <span class="detail-value pre-wrap">{{
                formData.describe_exposure || "N/A"
                }}</span>
            </div>
            ...
            <template v-for="symptom in stisymptoms" :key="symptom.key">
              <div v-if="formData[symptom.key] === 'Yes'" class="symptom-group">
                ...
                <div class="detail-item">
                  <span class="detail-label">Treatment/Referral:</span><span class="detail-value">{{
                    formData[symptom.key + "_treatment"] || "N/A"
                    }}</span>
                </div>
              </div>
            </template>
            ...
            <div class="detail-item">
              <span class="detail-label">Acute HIV Specify:</span><span class="detail-value">{{
                formData.acute_hiv_specify || "None"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">STI Specify:</span><span class="detail-value">{{
                formData.sti_specify || "None"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">Others Specify:</span><span class="detail-value">{{
                formData.other_specify || "None"
                }}</span>
            </div>

            <!-- Section 4: Disease History & Meds -->
            <h4 class="details-section-title">
              4. Disease History & Medications
            </h4>
            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-notes-medical"></i> Disease History:</span>
              <span class="detail-value">{{
                formData.disease_history || "N/A"
                }}</span>
            </div>
            <div v-if="formData.disease_history === 'Yes'" class="detail-item full-width">
              <span class="detail-label">History Details:</span>
              <span class="detail-value pre-wrap">{{
                formData.disease_specify || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-pills"></i> Current Medications:</span>
              <span class="detail-value">{{
                formData.current_medications || "N/A"
                }}</span>
            </div>
            <div v-if="formData.current_medications === 'Yes'" class="detail-item full-width">
              <span class="detail-label">Medication Details:</span>
              <span class="detail-value pre-wrap">{{
                formData.medication_specify || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-allergies"></i> Medication Allergies:</span>
              <span class="detail-value">{{
                formData.medication_allergies || "N/A"
                }}</span>
            </div>
            <div v-if="formData.medication_allergies === 'Yes'" class="detail-item full-width">
              <span class="detail-label">Allergy Details:</span>
              <span class="detail-value pre-wrap">{{
                formData.allergy_specify || "N/A"
                }}</span>
            </div>

            <!-- Section 5: Test Results -->
            <h4 class="details-section-title">
              5. Test Results (Sample Dates)
            </h4>
            <div class="detail-item">
              <span class="detail-label">HIV Rapid:</span><span class="detail-value">{{
                formData.hiv_rapid_sample_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">HIV 4th Gen:</span><span class="detail-value">{{
                formData.hiv_4th_sample_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">Hep B (HBsAg):</span><span class="detail-value">{{
                formData.hepatitis_b_sample_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">Hep C (Anti-HCV):</span><span class="detail-value">{{
                formData.hepatitis_C_sample_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">Syphilis:</span><span class="detail-value">{{
                formData.syphilis_sample_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">Kidney (Creatinine):</span><span class="detail-value">{{
                formData.kidney_function_sample_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">Gonorrhoea:</span><span class="detail-value">{{
                formData.gonorrhoea_sample_date || "N/A"
                }}</span>
            </div>
            ...
            <div class="detail-item full-width">
              ...
              <span class="detail-value pre-wrap">{{
                formData.assessment_notes || "None"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-box-open"></i> PrEP Dispensed:</span>
              <span class="detail-value">{{
                formData.prep_dispensed || "N/A"
                }}</span>
            </div>
            <div v-if="formData.prep_dispensed === 'Yes'" class="detail-item">
              <span class="detail-label">Bottles Dispensed:</span>
              <span class="detail-value">{{
                formData.prep_bottles ?? "N/A"
                }}</span>
            </div>

            <!-- EDIT 1: ADDED THIS BLOCK to display the regimen -->
            <div v-if="formData.prep_dispensed === 'Yes'" class="detail-item">
              <span class="detail-label">Regimen for PrEP:</span>
              <span class="detail-value">{{
                formData.regimen_for_prep || "N/A"
                }}</span>
            </div>
            <!-- END OF EDIT 1 -->

            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-calendar-check"></i> Clinician Info
                Date:</span>
              <span class="detail-value">{{
                formData.clinician_information_date || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label"><i class="fas fa-user-md"></i> Clinician Name:</span>
              <span class="detail-value">{{
                formData.clinician_name || "N/A"
                }}</span>
            </div>
            <div class="detail-item">
          ...
          <!-- EDIT 2: REMOVED THE EXTRA '<' CHARACTER THAT WAS BREAKING THE TEMPLATE -->
...
<script setup>
// EDIT 3: REMOVED UNNECESSARY IMPORTS. Vue automatically handles these.
...
const stisymptoms = [
  ...
  { key: "rash_data_resolved", label: "Rash (STI Section)" },
