...
const updateLocations = () => {
  if (allRows.value.length > 0) {
    ...
    if (locationIndex !== -1) {
      const uniqueLocations = new Set(allRows.value.slice(1).map(row => row[locationIndex]))
...
const applyFilters = () => {
  ...
  filteredRows.value = allRows.value.filter((row, index) => {
    ...
    const startDateMatch = !startDate.value || rowDate >= new Date(startDate.value);
    const endDateMatch = !endDate.value || rowDate <= new Date(endDate.value);
    const locationMatch = !selectedLocation.value || row[locationIndex] === selectedLocation.value;
    ...
    if (userRole === 'admin' || userRole === 'L1' || userRole === 'L2') {
      ...
      return startDateMatch && endDateMatch && locationMatch && row[locationIndex] === prepLocation;
    } else {
      return false;
    }
  });
...
const headerStatsData = computed(() => {
  ...
  if (interestedPrepIndex === -1 || suitableForPrepIndex === -1 || initiationDateIndex === -1) {
...
const kpiData = computed(() => {
  ...
  if (statusM1Index === -1 || statusM3Index === -1 || statusM6Index === -1 || statusM9Index === -1 || statusM12Index === -1) {
...
const lineChartData = computed(() => {
  if (!dataLoaded.value) return { series: [{ data: [] }], chartOptions: { xaxis: { categories: [] } } };
  ...
  if (dateIndex === -1 || kpGroupIndex === -1) {
    ...
    return { series: [{ data: [] }], chartOptions: { xaxis: { categories: [] } } };
  }
  ...
  filteredRows.value.slice(1).forEach(row => {
    ...
    dateKpGroupCounts[date][kpGroup] = (dateKpGroupCounts[date][kpGroup] || 0) + 1;
  });

  // Sort dates to ensure chronological order
  const sortedDates = Object.keys(dateKpGroupCounts).sort((a, b) => new Date(a) - new Date(b));

  // Find all unique KP groups in the data
  const kpGroups = new Set();
  sortedDates.forEach(date => {
    Object.keys(dateKpGroupCounts[date]).forEach(kpGroup => kpGroups.add(kpGroup));
  });
...
const donutChartsData = computed(() => {
  ...
  const columns = [
   'HIV 4th','HIV 4th_m3','HIV 4th_m6','HIV 4th_m9','HIV 4th_m12',
   'HIV 3rd','HIV 3rd_m3','HIV 3rd_m6','HIV 3rd_m9','HIV 3rd_m12',
   'Hepatitis B','Hepatitis B_m12',
   'Hepatitis C','Hepatitis C_m12',
   'Syphilis' ,'Syphilis_m6','Syphilis_m12',
   'Kidney','Kidney_m6','Kidney_m12',
   'Gonorrhoea','Gonorrhoea_m6','Gonorrhoea_m12',
  ];
//-	Gonorrhea 
