...
const sheetId =
  "2PACX-1vSfG6e5EIcHDaXopn9DxMZnTwVFGi5CiQxmKlEIPsd7uPtZiQIikYb46UdN78UhZlJfocCfl_s0hGGX";
const gid = "0";
...
const COLUMN_INDICES = {
  UID: 1, // Column A
  DATE_OF_REFERRAL: 4, // Column C
  PREP_SITE: 3, // Column D - Correct column for "Referral to PrEP site"
};
...
const fetchSheetData = async () => {
  try {
    ...
    const prepSiteColumnIndex = headerRow.findIndex(
      (header) => header.trim().toLowerCase() === "referral to prep site",
    );
    ...
    if (hasFullAccess.value) {
      ...
      allRows.value = [
        ...
        ...parsedRows
          .slice(1)
          .filter(
            (row) => row[COLUMN_INDICES.PREP_SITE] === user.prep_location,
          ),
      ];
    }
...
const updatePrepSiteOptions = () => {
  ...
  if (hasFullAccess.value) {
    ...
    prepSiteOptions.value = userData.value?.prep_location
      ? [userData.value.prep_location]
      : [];
...
const applyFilters = () => {
  filteredRows.value = allRows.value.filter((row, index) => {
    ...
    if (!hasFullAccess.value) {
      // For non-admin users, only show their assigned location
      prepSiteMatch =
        row[COLUMN_INDICES.PREP_SITE] === userData.value?.prep_location;
