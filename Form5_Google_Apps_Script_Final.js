...
function handleRequest(e) {
  ...
  try {
    console.log('=== FORM5 UNSCHEDULED VISIT HANDLER ===');
    ...
    switch (action) {
      case 'checkData':
        ...
        console.log('Checking data for UID:', uidForCheck);
      ...
      case 'prefill':
        ...
        console.log('Prefill request for UID:', uid);
      ...
      case 'delete':
        ...
        console.log('Delete request for UID:', deleteUid);
...
function getColumnMapping() {
  return {
    ...
    13: 'Abdominalcramps_Resolved', // Note: your header shows 'Abdominalcramps_ Resolved' with space
    14: 'Fatigue',
    15: 'Fatigue_Ongoing',
    16: 'Fatigue_Resolved',
    17: 'Dizziness',
    ...
    30: 'stop_or_continue',
    31: 'stopping_reason',
    32: 'stopping_indicatte_reasons_preferred_method', // Note: your header shows 'stopping_indicatte_reasons_ preferred_method' with space
    33: 'Other_notes',
...
function getPrefillData(uid) {
  try {
    console.log('getPrefillData called for UID:', uid);
...
function addNewRecord(e) {
  try {
    ...
    const uid = safeGetParameter(e, 'uid');
    console.log('Adding record for UID:', uid);
    ...
    if (existingData) {
      return { 
        status: 'error', 
        message: `Unscheduled visit record already exists for UID: ${uid}. Use update instead.` 
      };
    }
...
function deleteRecord(uid) {
  try {
    ...
    console.log('Deleting record for UID:', uid);
  ...
  try {
    ...
    if (data.length > 1) {
      ...
      if (testUid) {
        ...
        const prefillData = getPrefillData(testUid);
