...
function handleRequest(e) {
  ...
  try {
    console.log('=== CLINICAL FORM HANDLER DEBUG ===');
...
// Column mapping for prefill (based on your exact spreadsheet headers)
function getColumnMapping() {
  return {
    ...
    19: 'Resolved', // Dizziness resolved (missing prefix in header)
    20: 'Headache',
    21: 'Headache_Ongoing',
    ...
    30: 'stop_or_continue',
    31: 'stopping_reason',
    32: 'stopping_indicatte_reasons_ preferred_method', // Note: space in header
    33: 'Other_notes',
    34: 'Date', // Interviewer date
    35: 'Name', // Interviewer name
    36: 'Designation',
    37: 'BMHC Number' // Note: without colon
  };
}

// Get prefill data for specific UID only
function getPrefillData(uid) {
  try {
    console.log('getPrefillData called - UID:', uid);
    
    const data = fetchByUid(uid);
    if (!data) return { status: 'error', message: `No clinical data found for ${uid}` };
    ...
    const fieldMapping = {
      ...
      'stopping_indicatte_reasons_ preferred_method': 'stopping_prevention_method',
...
// Create the row data array for clinical form (matching spreadsheet column order)
...
function addNewRecord(e) {
  try {
    console.log('=== ADD NEW CLINICAL RECORD ===');
    
    const uid = safeGetParameter(e, 'uid');
    
    console.log('Add parameters - UID:', uid);
    ...
    if (existingData) {
      return { status: 'error', message: `Clinical record already exists for UID: ${uid}. Use update instead.` };
    }
    ...
    return { status: 'success', message: `Clinical Form submitted successfully for UID: ${uid}` };
  } catch (error) {
...
function updateRecord(e) {
  try {
    console.log('=== UPDATE CLINICAL RECORD ===');
    
    const uid = safeGetParameter(e, 'uid');
    
    console.log('Update parameters - UID:', uid);
    ...
    const existingData = fetchByUid(uid);
    if (!existingData) {
      return { status: 'error', message: `Clinical record for UID ${uid} not found` };
    }
    ...
    return { status: 'success', message: `Clinical form updated for UID: ${uid}` };
  } catch (error) {
...
function deleteRecord(uid) {
  try {
    console.log('=== DELETE CLINICAL RECORD ===');
    console.log('Delete parameters - UID:', uid);
    ...
    return { status: 'success', message: `Clinical record deleted for UID: ${uid}` };
  } catch (error) {
